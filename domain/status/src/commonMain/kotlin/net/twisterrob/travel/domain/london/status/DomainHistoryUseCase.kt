...
class DomainHistoryUseCase(
	...
	override fun history(feed: Feed, max: Int, current: Boolean, skipErrors: Boolean): List<ParsedStatusItem> {
		...
		return result
			.map { it.parse() }
			.filterNot { skipErrors && it is ParsedStatusItem.ParseFailed }
	}
